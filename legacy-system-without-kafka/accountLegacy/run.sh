...
echo "********************************************************"
echo "Waiting for the mysql server to start on port $MYSQL_PORT"
echo "********************************************************"
while ! `nc -z mysqlsrv $MYSQL_PORT`; do sleep 3; done
echo "*******  Mysql Server has started"



echo "********************************************************"
echo "Starting Account Legacy Service                         "
echo "********************************************************"
java -Djava.security.egd=file:/dev/./urandom -Dserver.port=$SERVER_PORT   \
     -Dspring.profiles.active=$PROFILE                                   \
     -jar /usr/local/accountLegacyservice/accountLegacy.jar
