...
echo "********************************************************"
echo "Waiting for the Catalog server to start on port $CATALOG_PORT"
echo "********************************************************"
while ! `nc -z catalogservice $CATALOG_PORT`; do sleep 3; done
...
java -Djava.security.egd=file:/dev/./urandom -Dserver.port=$SERVER_PORT   \
     ...
     -jar /usr/local/orderservice/order.jar
